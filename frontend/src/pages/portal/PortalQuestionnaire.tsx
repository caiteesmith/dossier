...
const inputStyle = {
  width: '100%', background: 'white', border: '1px solid #ddd', borderRadius: '8px',
  padding: '10px 14px', fontSize: '14px', color: '#1a1a2e', outline: 'none',
  fontFamily: 'inherit', boxSizing: 'border-box' as const,
}
const textareaStyle = { ...inputStyle, resize: 'vertical' as const, minHeight: '80px', lineHeight: '1.5' }
const labelStyle = { display: 'block', fontSize: '13px', fontWeight: 500, color: '#444', marginBottom: '6px' }
const hintStyle = { fontSize: '12px', color: '#aaa', marginTop: '4px', lineHeight: '1.4' }
...
function FieldInput({ field, value, onChange }: {
  ...
  switch (field.type) {
    case 'textarea':
      return <textarea value={strVal} onChange={e => onChange(e.target.value)} placeholder={field.placeholder} style={textareaStyle} />
    case 'date':
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {field.options?.map(opt => (
            <label key={opt} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', cursor: 'pointer' }}>
              <input type="radio" name={field.id} value={opt} checked={strVal === opt} onChange={() => onChange(opt)} style={{ marginTop: '2px', accentColor: '#0d1525' }} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {field.options?.map(opt => (
            <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
              <input type="checkbox" checked={arrVal.includes(opt)}
                onChange={e => { if (e.target.checked) onChange([...arrVal, opt]); else onChange(arrVal.filter(v => v !== opt)) }}
    ...
    default:
      return <input type="text" value={strVal} onChange={e => onChange(e.target.value)} placeholder={field.placeholder} style={inputStyle} />
  }
}

export default function PortalQuestionnaire({ booking, token }: Props) {
  const [answers, setAnswers] = useState<QuestionnaireAnswers>({
    partner_one_full_name: booking.partnerOneName ?? '',
    partner_two_full_name: booking.partnerTwoName ?? '',
    email_both: booking.email ?? '',
    phone: booking.phone ?? '',
    married_surname: booking.marriedSurname ?? '',
    mailing_address: [booking.mailingAddress, booking.mailingCity, booking.mailingState, booking.mailingZip]
      .filter(Boolean).join(', '),
  })
  ...
  useEffect(() => {
    api.get(`/api/portal/${token}/questionnaire`, noAuth)
      .then(r => {
        if (r.data.answers && Object.keys(r.data.answers).length > 0) {
          setAnswers(r.data.answers)
        }
        if (r.data.submittedAt) setSubmittedAt(r.data.submittedAt)
      })
      .catch(() => {}) // no answers yet — start blank
  ...
  async function saveProgress(submit = false) {
    ...
    try {
      const r = await api.post(
        `/api/portal/${token}/questionnaire`,
        { answers, submit },
  ...
  if (loading) {
    return (
      <div style={{ textAlign: 'center', padding: '60px 0' }}>
        <p style={{ fontSize: '14px', color: '#aaa' }}>Loading your questionnaire...</p>
      </div>
    )
  }
      ...
      <div style={{ textAlign: 'center', padding: '60px 0' }}>
        ...
        <div style={{ marginTop: '24px' }}>
          <button
            ...
            style={{
              fontSize: '14px', fontWeight: 500, color: '#0d1525',
          ...
          >
            ✏️ Review or update your answers
          </button>
        </div>
      </div>
    )
    ...
    <div>
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ fontSize: '13px', color: '#888', lineHeight: '1.5' }}>
          This helps your photographer understand your vision, timeline, and all the details that make your day unique. Take your time — your progress is saved as you go.
        </p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '4px', marginBottom: '32px', flexWrap: 'wrap' }}>
        {WEDDING_QUESTIONNAIRE.map((section, i) => (
          <button key={section.id} onClick={() => setActiveSection(i)} style={{
        ...
        <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          {currentSection.fields.map(field => (
            <div key={field.id}>
              ...
              <FieldInput field={field} value={answers[field.id] ?? ''} onChange={val => handleChange(field.id, val)} />
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <button onClick={() => setActiveSection(s => s - 1)} disabled={isFirst} style={{
          padding: '10px 20px', borderRadius: '8px', fontSize: '13px', fontWeight: 500,
          border: '1px solid #ddd', background: 'white', color: isFirst ? '#ccc' : '#555',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
          <button
            ...
            style={{
              ...
              border: '1px solid #ddd', background: 'white', color: saving ? '#bbb' : '#666',
        ...
        {isLast ? (
          <button onClick={handleSubmit} disabled={saving} style={{
            ...
            border: 'none', background: saving ? '#aaa' : '#0d1525', color: 'white',
            cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
          }}>
